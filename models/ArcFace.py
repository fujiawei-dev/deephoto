...
LastEditTime: 2021-01-09 22:09:11
'''
...
def ArcFaceModel():
    ...
    arcface_model = keras.layers.Dense(512, activation=None, use_bias=True,
                                       kernel_initializer="glorot_normal")(arcface_model)
    embedding = keras.layers.BatchNormalization(momentum=0.9, epsilon=2e-5,
...
def ResNet34():
    _input = tensorflow.keras.layers.Input(shape=(112, 112, 3))

    x = tensorflow.keras.layers.ZeroPadding2D(padding=1, name='conv1_pad')(_input)
    ...
    model = training.Model(_input, x, name='ResNet34')
